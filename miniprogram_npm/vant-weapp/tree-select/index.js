...
VantComponent({
  ...
  methods: {
    ...
    onClickNav(event) {
      ...
      if(item.topImg) {
        this.set({topImg: item.topImg})
        this.set({topLabel: item.topLabel})
      }
      if (!item.disabled) {
    ...
    updateSubItems() {
      ...
      const { children = [], topImg, topLabel } = items[mainActiveIndex] || {}
      this.updateItemHeight(children)
      return this.set({ subItems: children, topImg: topImg, topLabel: topLabel })
    },
